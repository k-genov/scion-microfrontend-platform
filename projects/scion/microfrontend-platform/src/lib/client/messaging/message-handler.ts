 ...
 * @ignore
 */
export class MessageHandler<IN extends Message, OUT> {
  ...
  constructor(message$: Observable<IN>, private _callback: (message: IN) => Observable<OUT> | Promise<OUT> | OUT | void) {
   ...
   */
  private consumeMessage(message: IN): void {
   ...
   */
  private handleMessage(request: IN): void {
